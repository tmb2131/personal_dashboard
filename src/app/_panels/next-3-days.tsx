    ...
    <section className="border-t border-border">
      ...
      <div className="px-5 pb-4">
        <div className="overflow-x-auto pb-1">
          <div className="grid min-w-[720px] grid-cols-1 gap-3 md:min-w-0 md:grid-cols-3">
            {groups.map((g) => (
              <div key={g.bucket.key} className="rounded-md border border-border bg-bg-elevated px-3 py-2.5">
                <div className="mb-2 flex items-baseline gap-2">
                  <span className="text-[13px] font-medium">{g.bucket.label}</span>
                  <span className="font-serif italic text-[11px] text-fg-muted">
                    {g.bucket.monthLabel}
                  </span>
                    ...
                    {g.events.map((e) => {
                      ...
                      return (
                        <li key={e.id} className="flex items-baseline gap-2 text-[12px]">
                          ...
                          {loc && (
                            <span className="hidden max-w-[8rem] shrink-0 truncate text-[10px] text-fg-subtle lg:inline">
                              {loc}
                            </span>
                          )}
                          {e.owner !== "other" && (
